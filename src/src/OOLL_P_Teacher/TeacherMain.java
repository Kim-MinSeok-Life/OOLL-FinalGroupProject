...
public class TeacherMain extends JFrame implements ActionListener { // JFrame 상속 + 버튼 액션 처리 위해 ActionListener 구현
    ...
    class LectureTableClickListener extends MouseAdapter { // MouseAdapter 상속
        ...
        public void mouseClicked(MouseEvent e) {          // 마우스 클릭 시 호출
            ...
            // 6번 열 → 수정 창 열기 (DB 연동은 TeacherLectureEdit 쪽에서 구현)
                ...
                // 클릭한 행의 lecture_no를 가져와서 같이 넘기기 위한 변수
                int lectureNo = lectureNoList.get(row); // 해당 행의 강의번호
                
                TeacherLectureEdit tle =                      // 기존 강의 수정 다이얼로그
                        new TeacherLectureEdit(TeacherMain.this, teacherNo, lectureNo);
    ...
    // ===== 강사가 담당하는 강의 목록 로드 =====
    // 검색어 없이 담당하는 강의 목록 불러오는 메소드
    private void loadLectureList() {
        loadLectureList(null); // 검색어 없이 전체목록
    }
    // 검색어(강의명) 기준으로 필터링한 강의목록 검색하는 메소드
    private void loadLectureList(String keyword) {
        ...
        try {
            ...
            if (hasKeyword){
                ps.setString(2, "%" + keyword + "%"); // 부분 일치 검색
            }
    ...
    public void refreshLectureList() {loadLectureList(); } // 내부에서 private 메소드 호출
    ...
    public void actionPerformed(ActionEvent ae) {         // 버튼 클릭 시 호출
        ...
        if (cmd.equals("로그아웃")) {                     // 로그아웃 버튼
            ...
            TeacherLectureCreate tlc =
                    new TeacherLectureCreate(this, teacherNo); // 다이얼로그 생성
            ...
            String keyword = searchField.getText().trim(); // 검색어 (공백제거)
            if (keyword.isEmpty())
                loadLectureList(); // 검색어 없으면 전체 강의 목록
            else
                loadLectureList(keyword);
        }
    } // actionPerformed
}
