...
// JDialog를 상속받아 모달 창(부모 창 제어 불가)으로 동작하며, ActionListener를 구현하여 버튼 이벤트를 처리합니다.
public class MyInfoDialog extends JDialog implements ActionListener {

    // --- UI 컴포넌트 변수 선언 ---
    JTextField idField;    // 아이디 (PK이므로 수정 불가 - 읽기 전용)
    JTextField nameField;  // 이름 (수정 가능)
    JTextField passField;  // 비밀번호 (수정 가능, 8자 이상 제한)
    ...
    String loginId; // 수정할 대상(로그인한 사람)의 아이디를 저장할 변수 (DB 조회 및 수정의 기준키)
    ...
    public MyInfoDialog(JFrame parent, String title, String id) {
        ...
        // (3) 비밀번호: 수정 가능 (유효성 검사에서 길이 체크 예정)
        ...
        // ★ 창이 열리자마자 DB에서 내 정보를 조회하여 필드에 채움 (사용자 편의성)
        loadMyInfo();
    }
    ...
    public void actionPerformed(ActionEvent e) {
        // 이벤트가 발생한 소스(버튼)가 무엇인지 확인
        if (e.getSource() == cancelBtn) {
            dispose(); // 취소 버튼 누르면 그냥 창 닫기
        } else if (e.getSource() == updateBtn) {
            // 수정 버튼 누르면 DB 업데이트 로직 실행 (예외 처리 포함)
            updateMyInfo();
        }
    }
    ...
    private void loadMyInfo() {
        ...
        try {
            ...
            // 2. SQL 작성: member 테이블에서 내 아이디(loginId)로 정보 조회
            String sql = "SELECT * FROM member WHERE member_id = ?";
            pstmt = con.prepareStatement(sql);
            pstmt.setString(1, loginId); // ? 에 로그인 ID 바인딩

            rs = pstmt.executeQuery(); // 쿼리 실행 (조회 결과는 ResultSet에 담김)
            ...
            // 4. 자원 해제 (연결 끊기 - 메모리 누수 방지)
            try { if(rs!=null) rs.close(); if(pstmt!=null) pstmt.close(); if(con!=null) con.close(); } catch(Exception ex) {}
        }
    }

    // ★ [기능 2] 정보 수정하기 (UPDATE 쿼리 + 예외 처리 패턴)
    private void updateMyInfo() {
        try {
            // 1. 입력값 유효성 검사 (검증 로직 분리 -> 문제 발생 시 예외 던짐)
            validateInputs();

            // 2. (검증 통과 시) DB 연결 및 업데이트 수행
            Connection con = null;
            PreparedStatement pstmt = null;
            try {
                // DB 연결
                ...
                pstmt = con.prepareStatement(sql);
                // 입력창에 있는 값들을 순서대로 SQL 파라미터(?)에 대입
                pstmt.setString(1, nameField.getText());
                pstmt.setString(2, passField.getText()); // 검사 통과한 비밀번호
                ...
                if (result > 0) {
                    ...
                    dispose(); // 성공 시 창 닫기 (MainFrame에서 자동으로 화면 갱신됨)
                } else {
                    // 쿼리는 실행됐지만 바뀐 게 없는 경우 (예: 아이디가 DB에 없음)
                    throw new Exception("수정 실패 (대상 정보를 찾을 수 없습니다)");
                }
    ...
    // ★ [사용자 정의 예외 클래스] 검증 실패 상황을 명확하게 표현하기 위해 만듦
